...
本研究では、ラスボス戦の感動体験の要素のうち、没入感と音楽演出に着目しました。
...
このようにインタラクティブミュージックという分野では、ゲームの状況に応じて音楽を変化させる取り組みが進められてきました。しかしその一方で、音楽に合わせてゲームの状況を調整するという逆のアプローチはまだあまり行われていません。そこで本研究では、BGMのクライマックスに合わせてちょうどラスボスが倒されるように、ラスボスの行動を制御するAIを構築します。
