...
また，音楽演出に加えて，プレイヤーの没入感を高める展開を意図的に起こし、印象的な場面を作り出します。
...
2つ目は，コマンド選択型のゲームは状態が不連続であり，強化学習の設計が比較的シンプルで扱いやすいと考えられること．
3つ目は，ボス戦がゲーム全体の中でも重要な局面であり，プレイヤーの記憶に強く残るからです．
...
強化学習とは，AIが環境の中で行動し，その結果得られる報酬をもとに，「どのように行動すれば最適かを学習する機械学習の一種です．
...
= 研究手法｜BGMと報酬関数
そのため，本研究では，曲のテンポや長さを変化させることは行わず，あらかじめ決まった音楽に合わせて，ボスの行動を制御します．
BGMはループの一番最後に最も盛り上がると仮定します．
灰色の縦の点線がループが終了するタイミングです．
報酬の関数をこのように設定します．
ループの終盤の近くでボスを倒すほど，点数が高くなりますが，少しでも超えると点数を大幅に下げます．
...
= 研究手法｜展開の動的制御
続いて，プレイヤーをさらに盛り上げる展開の題材案をいくつか説明します．
まず，攻撃の命中や回避，確率的に発生する高威力な攻撃、いわゆる会心の一撃などの運要素を取り入れ，これらを意図的に確率操作することをします．
...
次に，ゲーム序盤から中盤にかけて，プレイヤーを没入させる展開が発生した場合には，それに応じて報酬関数に加点を行います．
...
こうしたフィードバックをもとに，没入させる展開やイベントの内容を更新していきます．
